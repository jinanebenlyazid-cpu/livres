...
@section('title', 'Livres - BiblioTech Cloud')

@section('content')
<section class="container">
    <div class="row align-items-center g-4 mb-4">
        <div class="col-lg-7">
            <span class="text-primary fw-bold">Catalogue</span>
            <h1 class="section-title">Tous les livres</h1>
            <p class="text-muted">Recherchez par titre, auteur, categorie ou disponibilite.</p>
        </div>
        <div class="col-lg-5">
            <div class="stat-card p-4">
                <strong>{{ $livres->total() }}</strong>
                <span class="text-muted">resultats trouves</span>
            </div>
        </div>
    </div>
        ...
        @forelse($livres as $livre)
            <div class="col-md-6 col-xl-4">
                <div class="book-card h-100 p-3 fade-up">
                    ...
                    <p class="small text-muted">ISBN: {{ $livre->isbn }} | Exemplaires: {{ $livre->nombre_exemplaires }}</p>
                    <a href="{{ route('livres.show', $livre) }}" class="btn btn-outline-primary rounded-pill w-100">Voir details</a>
                </div>
            </div>
        @empty
            <div class="col-12">
                <div class="alert alert-info rounded-4">Aucun livre ne correspond a votre recherche.</div>
            </div>
        @endforelse
    </div>
